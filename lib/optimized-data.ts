...
type ApplicationsData = ReturnType<typeof getApplications>
type UsersData = ReturnType<typeof getUsers>
type UserAccessData = ReturnType<typeof getUserAccess>
...
export function getCachedApplications() {
  const cached = cache.get<ApplicationsData>(CACHE_KEYS.APPLICATIONS)
...
export function getCachedUsers() {
  const cached = cache.get<UsersData>(CACHE_KEYS.USERS)
  if (cached) return cached
...
export function getCachedUserAccess() {
  const cached = cache.get<UserAccessData>(CACHE_KEYS.USER_ACCESS)
  if (cached) return cached
...
export function getUserApplications(userId: number) {
  ...
  const cached = cache.get<ApplicationsData>(cacheKey)
  if (cached) return cached
...
export function getAdminData() {
  const cached = cache.get<{ users: UsersData; applications: ApplicationsData; userAccess: UserAccessData }>(CACHE_KEYS.ADMIN_DATA)
  if (cached) return cached
