...
export async function POST(request: NextRequest) {
  try {
    ...
    // Le mot de passe n'est jamais renvoyé au client (voir GET). Si le formulaire
    // est sauvegardé sans le re-saisir, on conserve l'ancien au lieu de l'effacer.
    if (!newConfig.password) {
      smtpConfig.password = current.password
    }

    // Active la config pour que sendEmail() (mot de passe oublié, activation,
    // notifications…) l'utilise réellement — sinon les vrais emails ne partent pas,
    // même si le bouton « Tester » fonctionne.
