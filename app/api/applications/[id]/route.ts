...
export async function PUT(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    return authError(error) || NextResponse.json({ error: "Erreur lors de la modification" }, { status: 500 })
  }
}
...
export async function DELETE(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    return authError(error) || NextResponse.json({ error: "Erreur lors de la suppression" }, { status: 500 })
  }
}
