...
export function LoadingSpinner({ 
  ...
  return (
    <div className={`flex flex-col items-center justify-center ${className}`}>
      <div className={`animate-spin rounded-full border-4 border-brand border-t-transparent ${sizeClasses[size]} mb-4`} />
      {text && (
        <p className={`text-ink ${textSizes[size]} font-medium`}>
          {text}
        </p>
      )}
    </div>
