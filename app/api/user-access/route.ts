...
export async function GET() {
  try {
    ...
    return authErrorResponse(error) || NextResponse.json({ error: "Erreur lors de la lecture" }, { status: 500 })
  }
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    return authErrorResponse(error) || NextResponse.json({ error: "Erreur lors de la modification" }, { status: 500 })
  }
}
